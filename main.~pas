...
type
  TMainForm = class(TForm)
    ...
    procedure crmConsoleMessage(Sender: TObject;
...
begin
  if crm.Browser <> nil then
  begin
    crm.Browser.MainFrame.ExecuteJavaScript(mmo1.Lines.Text, 'about:blank', 0);
    Sleep(250);
    tmr1.Enabled := True;
  end;
end;

procedure TMainForm.crmConsoleMessage(Sender: TObject;
  const browser: ICefBrowser; const message, source: ustring;
  line: Integer; out Result: Boolean);
var vMessage: String;
begin
  vMessage := message;
  if Copy(vMessage, 1, 1) = '^' then
  begin
     FMsgConsole := vMessage;
     FMsgConsole := StringReplace(FMsgConsole, '^', '', []);
     FMsgConsole := StringReplace(FMsgConsole, #9, ' ', [rfReplaceAll]);
     FMsgConsole := StringReplace(FMsgConsole, 'Â', ' ', [rfReplaceAll]);
     // não é espaço por espaço, mas sim alguns caraceteres malucos!!!
     FMsgConsole := StringReplace(FMsgConsole, ' ', ' ', [rfReplaceAll]);
     FMsgConsole := StringReplace(FMsgConsole, ' ', ' ', [rfReplaceAll]);
     FMsgConsole := StringReplace(FMsgConsole, '«', ' ', [rfReplaceAll]);
     FMsgConsole := StringReplace(FMsgConsole, '»', ' ', [rfReplaceAll]);
  end;
...
begin
  if Trim(FMsgConsole) <> '' then
  begin
     tmr1.Enabled := False;
     mmo2.Text := FMsgConsole;
  end;
end;
